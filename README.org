...
** Make it be Org!
Having scratch-buffer be org-mode makes it a lot more useful for writing ideas, etc. and still keeps it really useful for evaluating elisp!
#+begin_src elisp
#+end_src
...
* Elfeed
#+begin_src elisp
(when (string= home-user-name (user-login-name))
(use-package elfeed
  :commands elfeed
  :custom
